...
- **URL architecture**: do not assume the demo-prefixed route architecture is acceptable for the public live site. `pdk-creams.org` is the stakeholder-facing production domain and its intended public URL contract is clean direct routes (`/`, `/login`, `/contact`, `/dashboard`, etc.) with no visible `demo`, `uat`, `staging`, or `/creams/{demo_id}` markers in the production UX. If the codebase still contains `/creams/{demo_id}` routing, treat it as legacy/dev-compat behavior that must not leak into the official production user journey.
