...
{
    /**
     * Run the migrations - Trainee Management Module
     * Module: Trainee Management
     * Priority: 004 - High (Core User Group)
     */
    public function up(): void
    {
        // Courses table for trainee programs
        Schema::create('courses', function (Blueprint $table) {
            $table->id();
            $table->string('course_id')->unique();
            $table->string('course_name');
            $table->text('course_description');
            $table->integer('course_duration_weeks');
            $table->integer('max_participants');
            $table->text('course_objectives')->nullable();
            $table->json('required_skills')->nullable();
            $table->enum('difficulty_level', ['beginner', 'intermediate', 'advanced'])->default('beginner');
            $table->enum('course_status', ['active', 'inactive', 'draft', 'archived'])->default('active');
