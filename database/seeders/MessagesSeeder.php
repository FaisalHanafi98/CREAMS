...
class MessagesSeeder extends Seeder
{
    ...
    public function run()
    {
        try {
            ...
            // Get some users from each role for test messages using Users model with roles
            $admins = Users::where('role', 'admin')->where('status', 'active')->limit($mode === 'minimal' ? 1 : 2)->get();
            $supervisors = Users::where('role', 'supervisor')->where('status', 'active')->limit($mode === 'minimal' ? 1 : 2)->get();
            $teachers = Users::where('role', 'teacher')->where('status', 'active')->limit($mode === 'minimal' ? 2 : 3)->get();
            $ajks = Users::where('role', 'ajk')->where('status', 'active')->limit($mode === 'minimal' ? 1 : 2)->get();
