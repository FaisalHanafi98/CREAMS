                ...
                <div class="col-md-4 mb-3">
                    ...
                    <div class="position-relative">
                        <input type="text" class="form-control" id="search" name="search"
                            placeholder="Search by name, type, brand..." value="{{ request('search') }}">
                        <i class="fas fa-search position-absolute"
                            style="right: 15px; top: 50%; transform: translateY(-50%); color: #adb5bd;"></i>
                    </div>
                </div>
                        ...
                        @if (isset($centres))
                            @foreach ($centres as $centre)
                                <option value="{{ $centre->centre_name }}"
                                    {{ request('centre') == $centre->centre_name ? 'selected' : '' }}>
                                    {{ $centre->centre_name }}
                                </option>
                            @endforeach
                        @endif
                    ...
                    <tbody>
                        @forelse($assets ?? [] as $asset)
                            <tr>
                                <td>
                                    <div class="asset-info">
                                        <img src="{{ $asset->asset_avatar ? asset($asset->asset_avatar) : asset('images/default-asset.png') }}"
                                            alt="{{ $asset->asset_name }}" class="asset-image"
                                ...
                                <td>
                                    <span class="asset-type-badge">{{ $asset->asset_parent ?? 'N/A' }}</span>
                                </td>
                                <td>{{ $asset->asset_brand ?? 'N/A' }}</td>
                                <td>{{ $asset->centre_name ?? ($asset->center_name ?? 'Unassigned') }}</td>
                                <td>
                                    <span
                                        class="quantity-badge {{ $asset->asset_quantity <= 5 ? ($asset->asset_quantity == 0 ? 'empty' : 'low') : '' }}">
