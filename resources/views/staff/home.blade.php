...
<style>
    ...
    /* Role Colors - CREAMS Brand Colors */
    .role-admin {
    ...
    .role-teacher {
        background: linear-gradient(135deg, #32bdea, #0fa3cc) !important;
    }
    
    .role-ajk {
        background: linear-gradient(135deg, #c850c0, #a843a0) !important;
    }
...
<div class="container-fluid">
    ...
    <!-- Additional Row for AJK if there are any -->
    @if(($stats['ajks_count'] ?? 0) > 0)
    <div class="row">
        <div class="col-xl-3 col-md-6 mb-4">
            <div class="stats-card">
                <div class="stats-icon role-ajk">
                    <i class="fas fa-users fa-2x"></i>
                </div>
                <div class="stats-title">AJK/Committee</div>
                <div class="stats-value">{{ $stats['ajks_count'] ?? 0 }}</div>
            </div>
        </div>
    </div>
    @endif
        ...
        <div class="card-body">
            <form id="filter-form" action="{{ route('staffs.home') }}" method="GET">
                <div class="row">
                    ...
                    <div class="col-md-3 mb-3">
                        <label for="education-level-filter">Education Level</label>
                        <select class="form-control" id="education-level-filter" name="education_level">
                            <option value="">All Education Levels</option>
                            @foreach($educationLevels as $level)
                                <option value="{{ $level }}" {{ $filters['education_level'] == $level ? 'selected' : '' }}>
                                    {{ $level }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-3 mb-3">
                        <label for="specialisation-filter">Specialisation</label>
                        <select class="form-control" id="specialisation-filter" name="specialisation">
                            <option value="">All Specialisations</option>
                            @if(isset($specialisations) && $specialisations->count() > 0)
                                @foreach($specialisations as $specialisation)
                                    <option value="{{ $specialisation }}" {{ ($filters['specialisation'] ?? '') == $specialisation ? 'selected' : '' }}>
                                        {{ $specialisation }}
                                    </option>
                                @endforeach
                            @endif
                    ...
                    <div class="col-md-3 mb-3">
                        <label for="center-filter">Centre</label>
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <ul class="nav nav-tabs" id="activityTabs" role="tablist">
                ...
                @foreach($educationLevels as $index => $activity)
                    <li class="nav-item">
            ...
            <div class="tab-content mt-4" id="activityTabsContent">
                                    ...
                                    <div class="teacher-body">
                                        ...
                                        <ul class="teacher-details">
                                            ...
                                            <li>
                                                ...
                                                <span class="detail-value">{{ $user->education_specialization ?? 'N/A' }}</span>
                                            </li>
                                            <li>
                                                <span class="detail-label">Teaching:</span>
                                                <span class="detail-value">{{ $user->teaching_specialization ?? 'N/A' }}</span>
                                            </li>
                                            <li>
                                                <span class="detail-label">Centre:</span>
                                                <span class="detail-value">{{ $user->centre_name ?? 'Not Assigned' }}</span>
                                            </li>
                                        </ul>
                                    </div>
                ...
                @foreach($educationLevels as $index => $activity)
                    <div class="tab-pane fade" id="activity-{{ $index }}" role="tabpanel">
                        <div class="row">
                            @php
                                $activityUsers = $users->where('education_level', $activity);
                            @endphp
...
$(document).ready(function() {
    ...
    const roleChart = new Chart(roleCtx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: ['#dc3545', '#fd7e14', '#32bdea', '#c850c0'],
                hoverBackgroundColor: ['#c82333', '#e55a00', '#0fa3cc', '#a843a0'],
    ...
    const centreData = @json($stats['centre_breakdown'] ?? []);
    const centreLabels = Object.keys(centreData);
    const centreValues = Object.values(centreData);
    
    const centreCtx = document.getElementById('centerChart').getContext('2d');
    const centreChart = new Chart(centreCtx, {
        type: 'bar',
        data: {
            labels: centreLabels,
            datasets: [{
    ...
    const activityChart = new Chart(activityCtx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                data: activityValues,
                backgroundColor: [
                    '#32bdea', '#c850c0', '#fd7e14', '#dc3545', '#28a745',
                    '#6f42c1', '#17a2b8', '#32bdea', '#c850c0', '#fd7e14', 
                ],
                borderWidth: 1
            }]
        },
