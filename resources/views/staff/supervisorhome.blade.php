            ...
            <div class="box-container">
                ...
                @php
                    $usersInActivity1 = $users->where('user_activity_1', $activity_1)->sortBy(function ($userItem) {
                        if ($userItem->role === 'Supervisor') return 1;
                        elseif ($userItem->role === 'Teacher') return 2;
                        else return 3;
                    });
                @endphp

                @foreach ($usersInActivity1 as $index => $userItem)
                    @if ($userItem->role === 'Supervisor' || $userItem->role === 'Teacher')
                        @if ($index === 0)
                            <div class="box offer">
                                <div class="tutor">
                                ...
                                <a href="{{ route('user.supervisor.update', ['id' => $userItem->id]) }}"
                        ...
                        @else
                            <div class="box">
                                ...
                                <p>Major: <span>{{ $userItem->user_activity_1 }}</span></p>
                                <p>Minor: <span>{{ $userItem->user_activity_2 }}</span></p>
                                <a href="{{ route('user.update', ['id' => $userItem->id]) }}"
                                    class="inline-btn">View Profile</a>
                            </div>
                        @endif
                    @endif
