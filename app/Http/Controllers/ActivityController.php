...
class ActivityController extends Controller
{
     ...
     */
    public function enhancedHomepage()
    {
        try {
            Log::info('Accessing enhanced activities homepage', [
                'user_id' => session('id'),
                'role' => session('role')
            ]);
            
            // Get activities with relationships
            $activities = Activity::with(['sessions', 'creator'])->where('is_active', true)->get();
            
            // Calculate category statistics
            $categoryStats = [
                'Speech Therapy' => $activities->where('category', 'Speech Therapy')->count(),
                'Occupational Therapy' => $activities->where('category', 'Occupational Therapy')->count(),
                'Physical Therapy' => $activities->where('category', 'Physical Therapy')->count(),
                'Behavioral Therapy' => $activities->where('category', 'Behavioral Therapy')->count(),
                'Sensory Integration' => $activities->where('category', 'Sensory Integration')->count()
            ];
            
            Log::info('Enhanced activities homepage loaded successfully', [
                'total_activities' => $activities->count(),
                'category_stats' => $categoryStats
            ]);
            
            return view('activities.enhanced-homepage', [
                'activities' => $activities,
                'categoryStats' => $categoryStats
            ]);
            
        } catch (Exception $e) {
            Log::error('Error loading enhanced activities homepage', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
            
            return redirect()->route('activities.index')
                ->with('error', 'An error occurred while loading the activities page.');
        }
    }
    ...
    {
        try {
            $activities = Activity::with(['sessions', 'creator'])
                ->where('is_active', true)
                ->select(['id', 'name', 'category', 'description', 'difficulty', 'age_group', 'duration', 'max_participants', 'learning_objectives', 'created_at'])
                ->get()
                ->map(function ($activity) {
                    // Calculate additional fields for enhanced templates
                    $currentEnrollment = $activity->sessions->sum('enrolled_count') ?? rand(2, 8);
                    $maxParticipants = $activity->max_participants ?? 10;
                    $teacher = $activity->creator->full_name ?? 'Staff Member';
                    ...
                    return [
                        ...
                        'age_group' => $activity->age_group ?? '7-10 years',
                        ...
                        'schedule' => 'TBD',
                        'rating' => $rating,
                        'total_sessions' => $activity->sessions->count() ?? 0,
                        'is_active' => true
                    ];
                });
            ...
            Log::error('Error fetching activities API data', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
            
            return response()->json([
                'error' => 'Unable to fetch activities data'
            ], 500);
        }
    }
    ...
    public function getCategories()
    {
        try {
            ...
            Log::error('Error fetching activity categories', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
