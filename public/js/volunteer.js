...
function initializeVideoHero() {
    ...
    const heroSection = document.getElementById('videoHeroSection');
    ...
    // Fallback timeout - if video doesn't load in 3 seconds, hide it
    setTimeout(() => {
    ...
    // Fallback timeout - if video doesn't load within 3 seconds, hide it
    setTimeout(() => {
        if (video.readyState < 3) { // HAVE_FUTURE_DATA
            console.log('Video loading timeout, using fallback');
            video.style.display = 'none';
